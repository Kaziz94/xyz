export default _xyz => {
  ...
  function begin(params) {
    ...
    _xyz.mapview.interaction.edit.interaction = new ol.interaction.Modify({
      ...
      condition: e => {
        ...
        return !(_xyz.mapview.interaction.edit.trail && (mapp.utils.turf.kinks(mapp.utils.turf.flatten(_xyz.mapview.interaction.edit.trail).features[0]).features.length > 0));
  ...
  function update() {
    ...
    xhr.onload = e => {
      ...
      location.marker = ol.proj.transform( 
        mapp.utils.turf.pointOnFeature(feature.geometry).geometry.coordinates,  
  ...
  function contextmenu(e) {
    ...
    !_xyz.mapview.interaction.edit.info
      ...
      && menu.appendChild(mapp.utils.html.node`
        <li style="padding: 5px;" class="off-white-hover" onclick=${_xyz.mapview.interaction.edit.update}>${mapp.dictionary.layer_draw_update}</li>`);

    _xyz.mapview.interaction.edit.feature.length
      && menu.appendChild(mapp.utils.html.node`
        <li style="padding: 5px;" class="off-white-hover" onclick=${undo}>${mapp.dictionary.layer_draw_undo}</li>`);
  ...
  function validateFeature(e){
    ...
    if(mapp.utils.turf.kinks(_xyz.mapview.interaction.edit.trail).features.length > 0){

      _xyz.mapview.interaction.edit.info = mapp.utils.html.node`
        <div class="infotip" style="color:#d32f2f;">${mapp.dictionary.layer_draw_error}`;
