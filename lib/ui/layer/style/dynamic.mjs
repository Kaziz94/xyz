export default _xyz => layer => {
  ...
  // Grid SVG legend.
  const svg = legend.appendChild(mapp.utils.svg.node`<svg width=100% height=100px>`);
  ...
  for (let i = 0; i < n; i++) {
    ...
    svg.appendChild(mapp.utils.svg.node`
    <rect
      height=20
      width=${w + '%'}
      x=${x  + '%'}
    ...
    if (i === 0) {
      legend.color_min = svg.appendChild(mapp.utils.svg.node`
      <text
        class='label'
    ...
    if (i === (n / 2 % 1 != 0 && Math.round(n / 2) - 1)) {
      legend.color_avg = svg.appendChild(mapp.utils.svg.node`
      <text
        class='label'
    ...
    if (i === n - 1) {
      legend.color_max = svg.appendChild(mapp.utils.svg.node`
      <text
        class='label'
