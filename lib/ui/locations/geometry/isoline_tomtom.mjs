export default _xyz => {
  ...
  function createElements(entry) {
    ...
    if (entry.edit.defaults.traffic === undefined) entry.edit.panel.appendChild(mapp.utils.html.node `
    ...
    if (entry.edit.defaults.vehicleMaxSpeed === undefined) entry.edit.panel.appendChild(mapp.utils.html.node `
        <div style="padding-top: 5px; grid-column: 1 / 3">
  ...
  function create(entry) {
    ...
    _xyz
      .proxy(`https://api.tomtom.com/routing/1/calculateReachableRange/${ll[1]},${ll[0]}/json?${mapp.utils.paramString(entry.edit.isoline_tomtom)}&key={TOMTOM}`)
      .then(response => {
