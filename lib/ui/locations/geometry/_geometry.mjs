...
import isoline_tomtom from './isoline_tomtom.mjs'
...
export default entry => {
  ...
  function createContainers(entry) {
    ...
    entry.container = entry.listview.appendChild(mapp.utils.html.node `
      <div class="${`${entry.type} ${entry.class}`}">`)

    entry.container.appendChild(mapp.utils.html.node `
        ...
        <input type="checkbox"
          ...
          onchange=${e => {
            ...
            if (!entry.display && entry.edit) {
              ...
              if (entry.value && !entry.edit.skipConfirm && !confirm(mapp.dictionary.location_geometry_delete_msg)) return e.target.checked = true;
      ...
      !entry.style.theme && entry.container.appendChild(mapp.utils.html.node `
      <div
        class="sample-circle"
        style="${`
          background-color: ${mapp.utils.chroma(entry.style.fillColor
            || entry.style.strokeColor).alpha(entry.style.fillOpacity === undefined ? 1 : (parseFloat(entry.style.fillOpacity) || 0))};
          border-color: ${mapp.utils.chroma(entry.style.color || entry.style.strokeColor).alpha(1)};
          border-style: solid;
      ...
      entry.geometry = entry.value && _xyz.mapview.geoJSON({
        ...
        style: new ol.style.Style({
          stroke: entry.style.strokeColor && new ol.style.Stroke({
            color: mapp.utils.chroma(entry.style.color || entry.style.strokeColor).alpha(1),
            width: entry.style.strokeWidth || 1,
            lineDash: entry.style.lineDash
          }),
          fill: new ol.style.Fill({
            color: mapp.utils.chroma(entry.style.fillColor || entry.style.strokeColor).alpha(entry.style.fillOpacity === undefined ? 1 : parseFloat(entry.style.fillOpacity) || 0).rgba()
          })
        })
      });
