export default location => {
  ...
  //   location.mapview.Map.removeLayer(geom)
  // })
  ...
  for (const entry of location.infoj) {
      ...
      entry.listview.appendChild(mapp.utils.html.node`
        <div
          ...
          <span
            ...
            style="${`background-color: ${mapp.utils.chroma(location.style?.strokeColor || '#090').alpha(0.3)}`}">
              ${location.layer.name}`);
    ...
    entry.displayValue =
      entry.type === 'numeric' ? parseFloat(entry.value).toLocaleString('en-GB', { maximumFractionDigits: 2 }) :
        entry.type === 'integer' ? parseInt(entry.value).toLocaleString('en-GB', { maximumFractionDigits: 0 }) :
          entry.type === 'date' ? mapp.ui.formatDate(entry.value) :
            entry.type === 'datetime' ? mapp.ui.formatDateTime(entry.value) :
              entry.value
    ...
    if (entry.group) {
      ...
      if (!groups[entry.group]) {
        ...
        groups[entry.group] = mapp.utils.html.node`
        <div
    ...
    if (entry.plugin) {
      ...
      mapp.plugins[entry.plugin] && mapp.plugins[entry.plugin](entry)
    ...
    if (mapp.ui.locations.view[entry.type]) {

      console.log(entry.type)
      mapp.ui.locations.view[entry.type](entry);
      continue
    }
    ...
    entry.val = entry.listview.appendChild(mapp.utils.html.node`
      <div
    ...
    if (entry.query) {
      ...
      // mapp.query(entry).then(response => {
      //   if(!response) return
    ...
    if (entry.edit && !entry.fieldfx) {
      mapp.ui.locations.view.edit.input(entry)
      continue
    }
