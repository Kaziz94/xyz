export default _xyz => (params = {}) => {
  ...
  const z = mapp.hooks && mapp.hooks.current.z || locale.view && locale.view.z || locale.minZoom || 0

  const center = ol.proj.fromLonLat([
    parseFloat(mapp.hooks && mapp.hooks.current.lng || locale.view && locale.view.lng || 0),
    parseFloat(mapp.hooks && mapp.hooks.current.lat || locale.view && locale.view.lat || 0),
  ])
