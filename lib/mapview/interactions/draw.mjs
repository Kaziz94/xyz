export default function(params){
  ...
  // Finish current interaction on mapview.
  mapview.interaction?.finish()
  
  draw.srid = params.srid || mapview.srid
  ...
  const drawStroke = new ol.style.Stroke({
    color: params.style.strokeColor,
    width: params.style.strokeWidth
  })

  const drawFill = new ol.style.Fill({
    color: params.style.fillColor
  })
  ...
  mapview.node.addEventListener('contextmenu', contextmenu)
  ...
  function finish(feature) {
    ...
    mapview.node.removeEventListener('contextmenu', contextmenu)
  ...
  function contextmenu(e) {
    ...
    features.length && menu.appendChild(mapp.utils.html.node`
      <li
        style="padding: 5px;"
        class="off-white-hover"
        onclick=${()=>finish(getFeature())}>
          ${mapp.dictionary.layer_draw_save}`)
    ...
    !features.length && menu.appendChild(mapp.utils.html.node`
      <li
        ...
        onclick=${e=>{
          ...
          draw.vertices.pop()
          mapview.popup.remove()
        }}>${mapp.dictionary.layer_draw_last}`)
