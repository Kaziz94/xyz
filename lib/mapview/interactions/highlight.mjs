export default function(){
  ...
  async function singleClick(e) {
    ...
    // const location = await mapp.utils.xhr(mapview.host + '/api/location/get?' + mapp.utils.paramString({
    //   locale: mapview.locale.key,
    //   layer: interaction.layer.key,
    //   table: interaction.layer.table || interaction.layer.tableCurrent(),
    //   id: interaction.layer.highlight
    // }))

    if (interaction.feature.get('properties').count) return mapp.location.nnearest({
      mapview: mapview,
