...
function remove() {
  ...
  location.mapview.hooks && mapp.hooks.filter('locations', location.hook)

  delete location.mapview.locations.list[location.hook]

  location.Layers?.forEach(
    L => location.mapview.Map.removeLayer(L))

  // location.tabs?.forEach(
  //   tab => tab.remove()
  // )

  location.removeCallbacks?.forEach(
    fn => typeof fn === 'function' && fn(location))
}


async function get(location) {

  location.mapview = location.mapview || this
  ...
  location.marker = location.marker
    || ol.proj.transform(mapp.utils.turf.pointOnFeature(location.geometry).geometry.coordinates,
      'EPSG:' + location.layer.srid,
      'EPSG:' + location.mapview.srid)
...
async function select(location) {
  ...
  location.mapview.hooks && mapp.hooks.push('locations', location.hook)

  location.Layers.push(location.mapview.geoJSON(location))

  location.Marker = location.mapview.geoJSON({
    geometry: {
      type: 'Point',
  ...
  location.Layers.push(location.Marker)
...
async function nnearest(params) {
  ...
  params.mapview = params.mapview || this
