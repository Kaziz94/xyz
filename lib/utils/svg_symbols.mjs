export default marker => {
  ...
  function legend(style) {
    ...
    const node = mapp.utils.html.node `<div style="${_css}">`

    function make(m) {
      let css = `position:absolute; height:${m.cluster ? 40 : 20}px; width:${m.cluster ? 40 : 20}px;`
      if (m.svg) return node.appendChild(mapp.utils.html.node `<img style="${css}" src="${m.svg}"/>`)
      if (!m.type) return node.appendChild(mapp.utils.html.node `<img style="${css}" src="${dot({ color: '#666' })}"/>`)
      return node.appendChild(mapp.utils.html.node `<img style="${css}" src="${markers[m.type]}"/>`)
    }
...
function target(style) {
  ...
  <svg width=24 height=24 viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
    ...
    style.layers && Object.entries(style.layers).forEach(layer => {
        ...
        icon.appendChild(mapp.utils.svg.node `<circle cx=12 cy=12 r=${parseFloat(layer[0]) * 10} fill=${layer[1]}>`)
...
function triangle(style) {
  ...
  stroke-linejoin="round"/>`
    ...
    icon.appendChild(mapp.utils.svg.node `<polygon
...
function square(style) {
  ...
  <svg width=24 height=24 viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
    ...
    style.layers && Object.entries(style.layers).forEach(layer => {
        ...
        icon.appendChild(mapp.utils.svg.node `<rect fill=${layer[1]}
