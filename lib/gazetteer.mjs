export default params => {
  ...
  gazetteer.input.placeholder = mapview.locale.gazetteer.placeholder || '';
  ...
  gazetteer.input.addEventListener('keydown', e => {
    ...
    if (key === 46 || key === 8) {
      ...
      gazetteer.clear()
      gazetteer.layer && mapview.Map.removeLayer(gazetteer.layer)
      return
    }
  ...
  function search (term, params = {}){
    ...
    gazetteer.xhr.open('GET', mapview.host + '/api/gazetteer?' +
    ...
    gazetteer.xhr.onload = e => {
      ...
      if (e.target.response.length === 0) {
        gazetteer.result.appendChild(mapp.utils.html.node`
          <li>${mapp.dictionary.search_no_results}`)
      ...
      Object.values(e.target.response).forEach(entry => {
        ...
        ${mapview.locale.gazetteer.label ? mapp.utils.html.node`
          <span
            class="secondary-colour-bg" 
            style="font-size: smaller; padding: 0 2px 0 2px; cursor: help; border-radius: 2px;">
              ${(mapview.layers.list[entry.layer] && mapview.layers.list[entry.layer].name) || entry.layer || entry.source}</span>` : ``}
  ...
  function createFeature(geom){
    ...
    gazetteer.layer = mapview.geoJSON({
      geometry: geom,
      dataProjection: '4326',
      style: new ol.style.Style({
        image: mapp.utils.icon(gazetteer.icon)
      })
    })
  
    mapview.flyToBounds(gazetteer.layer.getSource().getExtent())
  ...
  function google(record) {
    ...
    mapp
      .utils
      .proxy(`https://maps.googleapis.com/maps/api/place/details/json?placeid=${record.id}&{GOOGLE}`, mapview.host)
      .then(response => {
        const feature = {
          type: 'Point',
