...
export default layer => {
  ...
  const source = new ol.source.Vector({
    loader: function (extent, resolution, projection) {
      ...
      layer.xhr.open(
        'GET', layer.mapview.host + `/api/layer/cluster/${layer.mapview.getZoom()}?` +
  ...
  function Style(feature) {
    ...
    if(marker.layers && Array.isArray(marker.layers)) {
      ...
      return marker.layers.map(l => {
        ...
        return new ol.style.Style({
          ...
          image: mapp.utils.icon(Object.assign({}, l, {
            scale: size/(l.size || layer.style.size) * (l.scale || 1) * (l.relative ?
              layer.mapview.getZoom()/(layer.mapview.locale.maxZoom-layer.mapview.locale.minZoom)
              : 1)
          }))
        })
      })
    ...
    if (layer.highlight === feature.get('id')){
      ...
      let tmpHighlightStyle = mapp.utils.lodash.cloneDeep(layer.style.highlight);
    ...
    const image = mapp.utils.icon(Object.assign({},
      marker,
      {
        scale: size/(marker.size || layer.style.size) * (marker.scale || 1) * (marker.relative ?
          layer.mapview.getZoom() / (layer.mapview.locale.maxZoom-layer.mapview.locale.minZoom)
          : 1)
      }))
