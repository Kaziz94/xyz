export default _xyz => location => {
  ...
  if (location._new) {
    ...
    location.marker = ol.proj.transform(
      mapp.utils.turf.pointOnFeature(location.geometry).geometry.coordinates,
  ...
  xhr.onload = e => {
    ...
    if (mapp.hooks) mapp.hooks.push('locations', location.hook)
    ...
    location.marker = ol.proj.transform(mapp.utils.turf.pointOnFeature(e.target.response.geometry).geometry.coordinates,
        'EPSG:' + location.layer.srid,
        'EPSG:' + _xyz.mapview.srid)
