export default (_xyz) => (entry) => {
  ...
  entry.value.features.map((feature) => {
    let style;
    if (!entry.style)
      style = entry.location.style;
    if (!entry.style.theme)
      style = entry.style;
    if (entry.style.theme && entry.style.theme.type === "categorized")
      style = entry.style.theme && entry.style.theme.cat[feature.properties[entry.style.theme.field]].style;
    if (entry.style.theme && entry.style.theme.type === "graduated") {
      for (let i = 0; i < entry.style.theme.cat_arr.length; i++) {
        if (feature.properties[entry.style.theme.field] < entry.style.theme.cat_arr[i].value)
          break;
    ...
    let f = _xyz.mapview.geoJSON({
      ...
      style: new ol.style.Style({
        stroke: style.strokeColor && new ol.style.Stroke({
          color: mapp.utils.chroma(style.color || style.strokeColor).alpha(1),
          width: entry.style.strokeWidth || 1
        }),
        fill: new ol.style.Fill({
          color: mapp.utils.chroma(style.fillColor || style.strokeColor).alpha(style.fillOpacity === void 0 ? 1 : parseFloat(style.fillOpacity) || 0).rgba()
        })
      })
    });
  ...
  entry.display = true;
  if (!entry.style || !entry.style.theme)
    return;
  if (!entry.style.theme.legend)
    return;
  entry.legend = mapp.utils.html.node`
      <div class="${`${entry.class} legend`}">`;
  Object.entries(entry.style.theme.cat).forEach((cat) => {
    entry.legend.appendChild(mapp.utils.svg.node`
    ...
    entry.legend.appendChild(mapp.utils.html.node`
        <div
