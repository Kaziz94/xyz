export default (_xyz) => {
  ...
  function settings(entry) {
    if (entry.edit.isoline_mapbox && !entry.edit.isoline_mapbox.geometry && (entry.edit.isoline_mapbox.minutes || !entry.edit.isoline_mapbox.minutes && entry.value))
      return;
    if (entry.edit.isoline_mapbox && entry.edit.isoline_mapbox.geometry && entry.edit.isoline_mapbox.minutes && !entry.value)
      return;
    panel(entry);
    if (entry.edit.isoline_mapbox && entry.edit.isoline_mapbox.geometry && (entry.edit.isoline_mapbox.minutes || !entry.edit.isoline_mapbox.minutes && entry.value)) {
      edit(entry);
    ...
    <div
      ...
      e.target.parentElement.classList.toggle("active");
            ...
            <ul>
                ${modes.map((keyVal) => mapp.utils.html.node`
                <li onclick=${(e) => {
  ...
  function create(entry) {
    ...
    xhr.open("GET", `https://api.mapbox.com/isochrone/v1/mapbox/${entry.edit.isoline_mapbox.profile || "driving"}/${ll.join(",")}?` + mapp.utils.paramString({
    ...
    xhr.onload = (e) => {
      ...
      if (entry.edit.isoline_mapbox.meta) {
        let date = new Date();
        entry.location.infoj.filter((_entry) => _entry.type === "meta" && _entry.field === entry.edit.isoline_mapbox.meta).forEach((meta) => meta.newValue = {
          provider: "Mapbox",
