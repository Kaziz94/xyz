export default (_xyz) => (layer) => {
  if (typeof layer.edit.isoline_here !== "object")
    layer.edit.isoline_here = {};
  layer.edit.defaults = {};
  ...
  function draw(e) {
    ...
    _xyz.mapview.interaction.draw.begin({
      ...
      geometryFunction: function(coordinates, geometry) {
        geometry = new ol.geom.Circle(coordinates, layer.edit.isoline_here.minutes * 1e3);
        ...
        _xyz.proxy(`https://isoline.router.hereapi.com/v8/isolines?${mapp.utils.paramString(params)}${layer.edit.isoline_here.maxPoints ? `&shape[${layer.edit.isoline_here.maxPoints}]` : ``}&{HERE}`).then((response) => {
          if (!response.isolines) {
            console.log(response);
            return alert(mapp.dictionary.here_error);
          }
          const coordinates2 = mapp.utils.isolineHereDecode(response.isolines[0].polygons[0].outer);
          coordinates2.polyline.map((p) => {
            return p.reverse();
          });
          coordinates2.polyline.push(coordinates2.polyline[0]);
          _xyz.mapview.interaction.draw.feature({
            geometry: {
              type: "Polygon",
              coordinates: [coordinates2.polyline]
            }
          });
        });
        return geometry;
  ...
  function createElements(group2) {
    ...
    group2.appendChild(makeDropdown({
