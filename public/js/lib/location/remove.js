export default (_xyz) => function() {
  ...
  location.tabs?.forEach((tab) => tab.remove());
  location.geometries.forEach((geom) => _xyz.map.removeLayer(geom));
  location.geometryCollection.forEach((geom) => _xyz.map.removeLayer(geom));
  if (location.Layer)
    _xyz.map.removeLayer(location.Layer);
  if (location.Marker)
    _xyz.map.removeLayer(location.Marker);
  if (typeof location.removeCallback === "function")
